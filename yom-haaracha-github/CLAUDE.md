...
מערכת ווב בעברית (RTL) לניהול יום הערכה למלגאים: ~40–50 נבחנים במקביל, כל אחד 4 פרקי מבחן + ריאיון אחד, פרוסים על **5 סבבים**. המקור המחייב: `בריף-בנייה-קלוד-קוד.md` (בתיקיית ההורה `~/Desktop/מבחנים ליום הערכה/`). **המשתמש אינו מתכנת — להסביר בעברית פשוטה.**
...
- **גוגל שיטס לייב**: `pushToSheet` בשרת (fire-and-forget, כבוי אם `sheetsWebhookUrl`/`SHEETS_WEBHOOK_URL` ריק) נורה ב-`submit-slot`. הגדרה: `docs/google-sheets-setup.md` + `config.local.json`.
- **בנק תוכן מלא יובא (30 פרקים)** ב-`content/` — 8 מקצועות × 3 וריאנטים (מתמטיקה 5/4/3 = 9). 8 פרקי הזרע הוסרו. מתמטיקה ב-KaTeX (`tex`/`\(...\)`), שבר עברי בטוקן `[[frac:...|...]]`. עבר `check-content` (0 שגיאות) + rehearsal 18/18 ל-N=30 ו-N=50, ובדיקת מקביליות אמיתית (30 בקשות בו-זמנית, 62ms).
...
2. **יזמות — גירלס פלוס**: להוסיף 3 פרקי `content/entrepreneurship_*.json` (subject חדש) כשהשאלות יגיעו — יופיע אוטומטית ב-`/api/subjects` אחרי restart.
3. "לא בנוח → רמה מתחת" במתמטיקה: מוגבל — כל נבחן עושה מתמטיקה פעם אחת, ואין "פרק מתמטיקה הבא" להורדת רמה. לשקול הורדת רמה בפרק הנוכחי.
