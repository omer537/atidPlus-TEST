...
1. **סימון מי לריאיון:** בטבלת הנבחנים, בעמודה **"לריאיון?"**, סמן/י בתיבות מי יוצא לריאיון בסבב הקרוב (השאר יעשו אוטומטית את הפרק הבא שלהם). אפשר גם לסמן מראש (ראה "העלאת תכנון" למטה). מיד רואים שתי רשימות חיות: **"בריאיון"** ו-**"בפרק"** — לפי שם.
...
**סטטוס לכל נבחן** (בטבלה): "עשה: … · נותר: … · התראיין: כן/לא", ומה הוא עושה עכשיו + הטיימר.

**פעולות פרטניות לכל נבחן** (כפתורים בשורה, עם שמות ברורים):
- **+2 דק׳** — הוספת שתי דקות לטיימר.
- **השהה / המשך** — עצירה והמשך של הטיימר שלו.
- **אפס לו** — התחלת המשבצת שלו מחדש (טיימר מאפס).
- **סיים לו** — סגירת המשבצת שלו עכשיו (עובר להמתנה).
- **✕** — הסרת הנבחן.
